      ...
      {tabs.map(t => (
        <button key={t.id} onClick={() => onChange(t.id)}
          style={{
            ...
            color: active === t.id ? P.purple : P.muted, cursor: 'pointer', fontSize: 15, fontWeight: active === t.id ? 800 : 400,
            boxShadow: active === t.id ? '0 1px 6px rgba(88,28,135,.10)' : 'none',
            transition: 'all .15s', fontFamily: 'Tajawal, sans-serif', whiteSpace: 'nowrap'
          }}>
          {t.label}
        </button>
      ))}
